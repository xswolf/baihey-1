...
class Area extends \common\models\Base
{
    ...
    {
        $area = $this->getArea();
        //var_dump($area);exit;//4,0,2,3,1,5,6
        foreach ($area as $key => $val) {
            ...
            $id = $this->addArea($data);
            if($val->name == '北京' || $val->name == '上海' || $val->name == '天津' || $val->name == '重庆') {
                $data['parentId'] = $id;
                $data['relation'] = $data['relation'] . ',' . $id;
                $uid = $this->addArea($data);
                $this->recursionArea($val->sub,$uid,$data['relation']);

            } else {
                $this->recursionArea($val->sub,$id,$data['relation']);
            }

            echo '地区'.$val->name.':'.$id.'插入完成<br/>';
        }
        exit;
    }
     ...
     * @param $relation
     */
    public function recursionArea($sub, $id, $relation) {
        foreach($sub as $val) {
            if($val->name != '不限') {
                $data['name'] = $val->name;
                $data['parentId'] = $id;
