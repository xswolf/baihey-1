...
define(['app/module', 'app/directive/directiveApi'
            ...
            $scope.moreImg = function (img, event) {
                ...
                var hideSheet = $ionicActionSheet.show({
                    ...
                    buttonClicked: function (i) {
                        // 设置头像
                        api.save('/wap/member/set-head', {id: img.id, thumb_path: img.thumb_path}).success(function (res) {
                            if (res.status < 1) {
        ...
        module.controller("member.security_pass", ['app.serviceApi', '$scope', '$timeout', '$ionicPopup', '$location', function (api, $scope, $timeout, $ionicPopup, $location) {
            ...
            $scope.saveData = function () {
                ...
                if (!$scope.formData.new_pass1) {
                    ar.saveDataAlert($ionicPopup, '请填写新密码');
                    return false;
                }
                if ($scope.formData.new_pass1.length < 6) {
                    ar.saveDataAlert($ionicPopup, '密码长度必须大于6个字符');
                    return false;
                }
                if ($scope.formData.new_pass1 != $scope.formData.new_pass2) {
                ...
                api.save('/wap/user/reset-password', $scope.formData).success(function (res) {
                    if (res.status) {
                        ...
                        ar.saveDataAlert($ionicPopup, '密码修改失败，可能原密码错误！');
                    }
                })
                            ...
                            if (!res.status) {
                                ...
                                api.save('/wap/user/update-user-data', {phone: $scope.formData.phone}).success(function (res) {
                                    if (res.data) {
                                        ...
                                        var alertPopup = $ionicPopup.alert({
                                            title: '重要提示',
                                            template: '绑定成功，您的初始密码为：' + ar.getPassByPhone($scope.formData.phone) + '，请及时修改您的密码。'
                                        });
                                        alertPopup.then(function (res) {
...
// 我的约会-参与的人
        ...
        module.controller("member.matchmaker", ['app.serviceApi', '$scope', '$timeout', '$ionicPopup', '$location', '$filter', function (api, $scope, $timeout, $ionicPopup, $location, $filter) {
            ...
            api.list("/wap/user/get-user-info", []).success(function (res) {
                ...
                } else if ($scope.userInfo.matchmaker && $scope.userInfo.matchmaker != '0') {
                    $scope.showMatchmaker = true;
                    $scope.title = '专属红娘';
