...
class ArrayGroupBy
{
    ...
    public function groupByField($group_field)
    {
        ...
        $grouped = call_user_func_array("\\common\\util\\ArrayGroupBy::groupByFieldDeep", $group_by_field_params);
    ...
    {
        if (!is_array($arr)) {
            trigger_error('\common\util\ArrayGroupBy::groupByFieldDeep(): The first argument should be an array', E_USER_ERROR);
        }
        if (!is_string($key) && !is_int($key) && !is_float($key)) {
            trigger_error('\common\util\ArrayGroupBy::groupByFieldDeep(): The key should be a string or an integer', E_USER_ERROR);
        }
    ...
    public static function arrayColumn($array, $column_key, $index_key = null)
    {
        if (!is_array($array) && !($array instanceof \ArrayAccess))
            throw new \Exception('Argument 1 passed to \common\util\ArrayGroupBy::::arrayColumn() must be of the type array');
